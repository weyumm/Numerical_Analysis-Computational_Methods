...
def print_backward_diff_table(x_list, diff_table):
    """打印向后差分表"""
    n = len(x_list)
    headers = ["x", "f(x)"]
    for i in range(1, n):
        headers.append(f"∇^{i}f")
...
def select_nodes(x, x_list, n_points):
    """选择以最接近x的右侧节点为终点的连续n_points个节点"""
    sorted_indices = np.argsort(x_list)
    x_list_sorted = x_list[sorted_indices]
    
    # 找到插入点
    idx = np.searchsorted(x_list_sorted, x)
    end = min(len(x_list_sorted) - 1, max(idx, n_points - 1))
    
    selected_indices = sorted_indices[end - n_points + 1 : end + 1]
    selected_x = x_list[selected_indices]
    
    return selected_x, selected_indices

def construct_backward_polynomial(selected_x, diff_row, h):
    """构造并展开后插多项式"""
    x = symbols('x')
    s = (x - selected_x[-1]) / h  # 从最后一个节点开始
    poly = 0
...
def calculate_and_print_result(x_val, selected_x, diff_row, h, polynomial):
    """计算插值点并输出过程"""
    x_sym = symbols('x')
    s = (x_val - selected_x[-1]) / h  # 从最后一个节点计算s
    value = 0
    
    print("\n【代入计算过程】")
    print("-" * 60)
    print(f"s = (x - xₙ)/h = ({x_val:.2f} - {selected_x[-1]:.1f}) / {h:.2f} = {s:.4f}")
    ...
    for k in range(len(diff_row)):
        ...
        print(f"第{k+1}项: ∇^{k}f/{factorial_k} * product = {diff_row[k]:.4f}/{factorial_k} * {term_s:.4f} = {term_value:.6f}")
...
def main(x_list, y_list, x, num_points=3):
    ...
    print_backward_diff_table(x_list, diff_table)
    
    # 选择节点
    selected_x, selected_indices = select_nodes(x, x_list, num_points)
    print(f"\n【使用节点】xₙ = {', '.join([f'{xi:.1f}' for xi in selected_x])}")
    
    # 获取差分值（从最后一个选中节点开始）
    diff_row = [diff_table[selected_indices[-1], k] for k in range(num_points)]
    h = selected_x[1] - selected_x[0]  # 计算步长
    print(f"步长 h = {h:.2f}")
    
    # 构造多项式
    polynomial = construct_backward_polynomial(selected_x, diff_row, h)
...
if __name__ == "__main__":
    ...
    # 示例：对 x=0.7 进行 3 点插值
    main(x_list, y_list, x=0.7, num_points=3)
    
    # 示例：对 x=0.65 进行 4 点插值
    # main(x_list, y_list, x=0.65, num_points=4)